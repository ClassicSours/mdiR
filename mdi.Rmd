...
```{r, echo = FALSE}
MDI <- data.frame(read.csv("~/mdi/maps.csv", stringsAsFactors = FALSE))
MDI$pool1 <- factor(MDI$pool1)
MDI$pool2 <- factor(MDI$pool2)
MDI$pool3 <- factor(MDI$pool3)
library(jpeg)
...
library(reshape2)
library(grid)
MDI.t <- MDI %>%                        select(dungeon,affix1,affix2,affix3) %>% melt('dungeon') %>% arrange(dungeon)
pool1 <- MDI %>% filter(pool1 == 1) %>% select(dungeon,affix1,affix2,affix3) %>% melt('dungeon') %>% arrange(dungeon)
pool2 <- MDI %>% filter(pool2 == 1) %>% select(dungeon,affix1,affix2,affix3) %>% melt('dungeon') %>% arrange(dungeon)
pool3 <- MDI %>% filter(pool3 == 1) %>% select(dungeon,affix1,affix2,affix3) %>% melt('dungeon') %>% arrange(dungeon)
...
poolToPlot(MDI.t,icons,"MDI")
```

```{r, fig.height = 3, echo = FALSE}
poolToPlot(pool1,icons,"Map Pool 1, Difficulty Level 22")
poolToPlot(pool2,icons,"Map Pool 2, Difficulty Level 23")
poolToPlot(pool3,icons,"Map Pool 3, Difficulty Level 24")
```
```{r}
finals <- read.csv("~/mdi/finals.csv", stringsAsFactors = FALSE)
...
poolToPlot(finals.p1,icons,"Finals Map Pool 1, Difficulty Level 23")
poolToPlot(finals.p2,icons,"Finals Map Pool 2, Difficulty Level 24")
poolToPlot(finals.p3,icons,"Finals Map Pool 3, Difficulty Level 25")
